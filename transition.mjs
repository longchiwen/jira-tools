const TransitionDefaultOptions = {
  name: "[Task name]",

  // Jira JQL syntax used for retrieving a list of issues
  searchJql: 'project = MY_JIRA_PROJECT AND status in ("STATUS")',

  // a list of transitions to run (first one to succeed wins)
...
export default class Transition {
  ...
  buildIssueUpdate(options) {
    ...
    if (typeof options.comment !== "undefined") {
      issueUpdate.comment = [
        {
          add: {
            body: this.asString(options.comment),
          },
        },
      ];
    }
  ...
  async runOne(options) {
    ...
    // construct options
    const opt = Object.assign({}, TransitionDefaultOptions, options);
    ...
    for (const o of list) {
      ...
      if (!transition) {
        this.error(
          `Unknown transition for ${
            o.id
          }. Available transitions: ${transitions.map((t) => t.name)}`
        );
        continue;
      }
      ...
      await this.transitionIssue(o.id, transition.id, opt);
    }
  ...
  async transitionIssue(issueKey, transitionId, issueUpdate) {
  ...
  detectTransitionId(transitionsList, options) {
    return transitionsList.find((o) => {
      if (options.transitions.indexOf(o.name.toLowerCase()) > -1) {
        return true;
      }
      return false;
    });
...
export const action = new Transition(client);
